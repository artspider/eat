          ...
          <div class="block lg:grid grid-cols-2 gap-4">
            <div class=" col-span-1 ">              
                ...
                <div class="mt-6">
                    ...
                    <div class="relative">
                      ...
                      <div x-show="open" x-on:click.away="open=false">
                        <div class=" z-10 absolute top-10 w-full bg-eat-green-100 mr-10 rounded">
                          ...
                          <li wire:click="SelectCategory({{$category}})" href="#" class="block cursor-pointer w-full text-sm bg-eat-olive-500 text-eat-white-500 font-light font-montserrat text-left px-4 py-2 hover:bg-eat-olive-100 hover:text-eat-white-100">{{$category->name}}</li>    
                          @else
                          <li x-on:click="open=false" wire:click="SelectCategory({{$category->id}})" href="#" class="block cursor-pointer w-full text-sm text-eat-olive-500 font-light font-montserrat text-left px-4 py-2 hover:bg-eat-olive-100 hover:text-eat-white-100">{{$category->name}}</li>                          
                          @endif
                  ...
                  <div class="mt-1 relative rounded-md shadow-md">
                    <select name="category" id="category" wire:model="category_id"
                      ...
                      <option class="select-option text-sm text-eat-olive-900  checked:bg-eat-olive-50 font-medium font-montserrat" value="{{$category->id}}">{{$category->name}}</option>
                      @empty                    
                      @endforelse
                    </select> 
                    @error('category')
                      <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                        <svg class="h-5 w-5 fill-current text-red-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16.143 2l5.857 5.858v8.284l-5.857 5.858h-8.286l-5.857-5.858v-8.284l5.857-5.858h8.286zm.828-2h-9.942l-7.029 7.029v9.941l7.029 7.03h9.941l7.03-7.029v-9.942l-7.029-7.029zm-6.471 6h3l-1 8h-1l-1-8zm1.5 12.25c-.69 0-1.25-.56-1.25-1.25s.56-1.25 1.25-1.25 1.25.56 1.25 1.25-.56 1.25-1.25 1.25z"/></svg>
                      </div>            
                    @enderror
                  </div> --}}
                </div>
                      ...
                      <div class="relative">
                        ...
                        <div x-show="open" x-on:click.away="open=false">
                          <div class=" z-10 absolute top-10 w-full bg-eat-green-100 mr-10 rounded">
                            ...
                            @else
                            <li x-on:click="open=false" wire:click="SelectUnit({{$unit->id}})" href="#" class="block cursor-pointer w-full text-sm text-eat-olive-500 font-light font-montserrat text-left px-4 py-2 hover:bg-eat-olive-100 hover:text-eat-white-100">{{$unit->unit}}</li>                          
                            @endif
                    ...
                    {{-- < div class="mt-1 relative rounded-md shadow-md">
                      <select name="unit" id="unit" wire:model="unit_id"
                        ...
                        @forelse ($units as $unit)
                          <option class="text-sm text-eat-olive-900 font-medium font-montserrat" value="{{$unit->id}}">{{$unit->unit}}</option>                    
                        @empty                    
                        @endforelse
                      </select>
                      @error('unit')
                ...
                <x-utils.text-input
                    ...
                    label="Presentación"                  
                    :required="true"
                    placeholder="Presentación, por ejemplo: Lata, botella, granel"
                    class="mt-6"
                />
                      ...
                      <div class="relative">
                        ...
                        <div x-show="open" x-on:click.away="open=false">
                          <div class=" z-10 absolute top-10 w-full bg-eat-green-100 mr-10 rounded">
                            ...
                            <li wire:click="SelectSupplier({{$supplier}})" href="#" class="block cursor-pointer w-full text-sm bg-eat-olive-500 text-eat-white-500 font-light font-montserrat text-left px-4 py-2 hover:bg-eat-olive-100 hover:text-eat-white-100">{{$supplier->company_name}}</li>    
                            @else
                            <li x-on:click="open=false" wire:click="SelectSupplier({{$supplier->id}})" href="#" class="block cursor-pointer w-full text-sm text-eat-olive-500 font-light font-montserrat text-left px-4 py-2 hover:bg-eat-olive-100 hover:text-eat-white-100">{{$supplier->company_name}}</li>                          
                            @endif
                ...
                <div class="mt-6">
                  <textarea placeholder="Descripción o detalles adicionales, por ejemplo: fecha de caducidad, número de serie"
                            ...
                            id="" 
                            cols="30" 
                            rows="5" wire:model="other"></textarea>
                </div>
